 ...
 */
export const getWalkInAppointments = async (receptionistUserId) => {
    const [rec] = await db
        .select()
        .from(receptionists)
    ...
    return db
        .select()
        .from(appointments)
        .where(and(eq(appointments.hospitalId, rec.hospitalId), eq(appointments.type, 'walk-in'), eq(appointments.status, 'pending'), isNull(appointments.receptionistId)));
};
/**
 * Confirm an appointment and notify doctor + patient.
 */
export const confirmAppointment = async (appointmentId, receptionistId) => {
