...
import { storage } from ".";
import { registrationSchema, loginSchema, otpVerificationSchema, insertHospitalSchema, insertDoctorSchema, insertPatientSchema, insertPrescriptionSchema, insertLabReportSchema } from "../shared/schema"; // <-- use relative import instead of alias
import { authenticateToken, authorizeRoles, generateToken } from "./middleware/auth";
import { seedDummyData } from "../scripts/seed-data";
import { db } from "./db";
import * as schema from "../shared/schema"; // <-- fix alias here
...
export async function registerRoutes(app) {
    ...
    app.post("/api/hospitals/profile", authenticateToken, authorizeRoles('hospital'), async (req, res) => {
        try {
    ...
    app.put("/api/hospitals/profile", authenticateToken, authorizeRoles('hospital'), async (req, res) => {
        try {
    ...
    app.post("/api/doctors/profile", authenticateToken, authorizeRoles('doctor'), async (req, res) => {
        try {
    ...
    app.post("/api/patients/profile", authenticateToken, authorizeRoles('patient'), async (req, res) => {
        try {
    ...
    app.get("/api/appointments/today", authenticateToken, authorizeRoles('doctor'), async (req, res) => {
        try {
            const doctor = await storage.getDoctorByUserId(req.user.id);
            if (!doctor) {
    ...
    app.post("/api/lab-reports", authenticateToken, authorizeRoles('lab'), async (req, res) => {
        try {
    ...
    app.get("/api/lab-reports/my", authenticateToken, async (req, res) => {
        try {
            let labReports = [];
            switch (req.user.role) {
                case 'patient':
    ...
    app.get("/api/doctors/by-hospital/:hospitalId", async (req, res) => {
        try {
            ...
            const doctors = await db.select({
                ...
                .where(and(eq(schema.doctors.hospitalId, hospitalId), eq(schema.doctors.isAvailable, true)));
            res.json(doctors);
        }
        catch (error) {
    ...
    app.get("/api/appointments/pending", authenticateToken, authorizeRoles('receptionist'), async (req, res) => {
        try {
            ...
            const pendingAppointments = await db.select({
                ...
                .where(and(eq(schema.appointments.hospitalId, receptionist.hospitalId), eq(schema.appointments.status, 'pending')))
    ...
    app.put("/api/appointments/:id/confirm", authenticateToken, authorizeRoles('receptionist'), async (req, res) => {
        try {
            ...
            switch (req.user.role) {
                ...
                case 'patient':
                    ...
                    if (patient) {
                        ...
                        stats.upcomingAppointments = appointments.filter(apt => new Date(apt.appointmentDate) > new Date() && apt.status === 'scheduled').length;
