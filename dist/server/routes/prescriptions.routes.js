...
router.get("/patient/filters", authenticateToken, authorizeRoles("PATIENT"), async (req, res) => {
    try {
...
router.put("/:id", authenticateToken, authorizeRoles("DOCTOR"), async (req, res) => {
    try {
        ...
        const updated = await prescriptionService.updatePrescription(req.user.id, prescriptionId, req.body);
        if (!updated)
