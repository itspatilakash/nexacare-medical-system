...
export default function PatientDashboard() {
    ...
    const headerActions = (<Button className="medical-blue text-white hover:bg-blue-700" onClick={() => setIsBookingModalOpen(true)}>
      Book Appointment
    </Button>);
    const upcomingAppointments = appointments?.filter((apt) => new Date(apt.appointmentDate) > new Date() && apt.status === 'scheduled') || [];
    return (<DashboardLayout title="John Smith" subtitle="Patient Portal" icon={<User className="w-6 h-6 text-white"/>} navigationItems={navigationItems} headerActions={headerActions}>
      {/* Health Overview */}
      ...
      <Card className="mb-6">
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Button variant="outline" className="h-20 flex flex-col items-center justify-center space-y-2 border-2 border-dashed border-gray-200 hover:border-medical-blue hover:bg-blue-50" onClick={() => setIsBookingModalOpen(true)}>
              <Calendar className="w-8 h-8 text-medical-blue"/>
              <span className="text-sm font-medium text-gray-900">Book Appointment</span>
            </Button>

            <Button variant="outline" className="h-20 flex flex-col items-center justify-center space-y-2 border-2 border-dashed border-gray-200 hover:border-medical-green hover:bg-green-50">
              <FileText className="w-8 h-8 text-medical-green"/>
              <span className="text-sm font-medium text-gray-900">View Prescriptions</span>
            </Button>

            <Button variant="outline" className="h-20 flex flex-col items-center justify-center space-y-2 border-2 border-dashed border-gray-200 hover:border-purple-500 hover:bg-purple-50">
              <ClipboardList className="w-8 h-8 text-purple-500"/>
              <span className="text-sm font-medium text-gray-900">Download Reports</span>
            </Button>

            <Button variant="outline" className="h-20 flex flex-col items-center justify-center space-y-2 border-2 border-dashed border-gray-200 hover:border-yellow-500 hover:bg-yellow-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="p-6">
            {appointmentsLoading ? (<div className="space-y-3">
              ...
              </div>) : upcomingAppointments.length > 0 ? (<div className="space-y-4">
                {upcomingAppointments.slice(0, 5).map((appointment) => (<div key={appointment.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
                    <div className="flex items-center">
                ...
                <Button className="mt-4 medical-blue hover:bg-blue-700" onClick={() => setIsBookingModalOpen(true)}>
        ...
        <Card>
          ...
          <CardContent className="p-6">
            {labReportsLoading ? (<div className="space-y-3">
                {[1, 2, 3].map((i) => (<div key={i} className="animate-pulse">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-gray-200 rounded-full"></div>
                      <div className="flex-1 space-y-2">
                ...
                {labReports.slice(0, 5).map((report) => (<div key={report.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
                    <div className="flex items-center">
